...
int X, Y, NUM_THREAD, local_row_num,local_size;
int * result_arr;
int MAG_THRE = 2;
int ITER_THRE = 100;
...
void *cal_M_set(void *t){
    int *my_id = (int *)t;
    for(int idx = local_size*(*my_id); idx<local_size*(*my_id+1);idx++){
        int i = idx/X;
        int j = idx%X;
...
//    printf("%d\n",NUM_THREAD);
    int array_size = X*X;
    local_size = array_size/NUM_THREAD;
    if(array_size%NUM_THREAD != 0){
        local_size+=1;
    }
    array_size = NUM_THREAD * local_size;
    ...
    printf("Assignment 2, Mandelbrot Set, Pthread static implementation.\n");
